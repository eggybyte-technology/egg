...
check_working_directory() {
    ...
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        ...
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
            ...
            # Check for large files before staging (only files that will be added)
            print_section "Checking for large files"
            if ! bash "$SCRIPT_DIR/check-large-files.sh" --check-staged; then
                exit_with_error "Large files detected in files to be committed. Please remove or use git-lfs before committing."
            fi
...
delete_all_tags() {
    ...
    echo ""
    print_error "⚠️  WARNING: This will delete ALL version tags for ALL modules (including CLI)!"
    ...
    # Find tags for framework modules
    for mod in "${ALL_MODULES[@]}"; do
...
main() {
    ...
    # Check for large files before release (only files that will be added)
    print_section "Checking for large files"
    if ! bash "$SCRIPT_DIR/check-large-files.sh" --check-staged; then
        exit_with_error "Large files detected in files to be committed. Please remove or use git-lfs before releasing."
    fi
