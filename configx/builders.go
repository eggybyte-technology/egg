// Package configx provides convenience functions for building configuration sources.
//
// Overview:
...
// DefaultManager creates a manager with sensible defaults.
// This is a convenience function for quick setup.
func DefaultManager(ctx context.Context, logger log.Logger) (Manager, error) {
	sources, err := BuildSources(ctx, logger)
	if err != nil {
		return nil, err
	}
...
// QuickBind is a convenience function for quick configuration binding.
// It creates a manager, binds the target, and returns the manager for further use.
func QuickBind(ctx context.Context, logger log.Logger, target any) (Manager, error) {
	manager, err := DefaultManager(ctx, logger)
	if err != nil {
		return nil, err
	}

	if err := manager.Bind(target); err != nil {
		return nil, err
	}
