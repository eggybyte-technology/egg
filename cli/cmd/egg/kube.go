...
var kubeGenerateCmd = &cobra.Command{
	Use:   "generate",
	Short: "Generate Helm charts",
	Long: `Generate Helm charts from egg configuration.

This command:
- Creates Helm chart structure for all services
- Generates deployment manifests
- Sets up ConfigMaps and Secrets
- Prepares Kubernetes resources

Example:
  egg kube generate`,
	RunE: runKubeGenerate,
}
...
// runKubeGenerate executes the kube generate command.
//
// Parameters:
//   - cmd: Cobra command
...
func runKubeGenerate(cmd *cobra.Command, args []string) error {
	ui.Info("Generating Helm charts...")
	...
	ui.Success("Helm charts generated successfully!")
	...
	ui.Info("Next steps:")
	ui.Info("  1. Review generated charts in deploy/helm/")
	ui.Info("  2. Template with namespace: egg kube template -n <namespace>")
	ui.Info("  3. Apply to cluster: egg kube apply -n <namespace>")
...
func applyHelmCharts(ctx context.Context, runner *toolrunner.Runner, config *configschema.Config, namespace string) error {
	// Apply unified project chart
	chartPath := filepath.Join("deploy/helm", config.ProjectName)
	releaseName := config.ProjectName

	ui.Info("Applying unified Helm chart for project: %s", config.ProjectName)
	if err := applyHelmChart(ctx, runner, chartPath, releaseName, namespace); err != nil {
		return fmt.Errorf("failed to apply unified chart: %w", err)
	}
...
func uninstallHelmCharts(ctx context.Context, runner *toolrunner.Runner, config *configschema.Config, namespace string) error {
	// Uninstall unified project chart
	releaseName := config.ProjectName

	ui.Info("Uninstalling unified Helm chart for project: %s", config.ProjectName)
	if err := uninstallHelmChart(ctx, runner, releaseName, namespace); err != nil {
		return fmt.Errorf("failed to uninstall unified chart: %w", err)
	}
